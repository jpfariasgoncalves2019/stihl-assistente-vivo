...
create function normalize_text(t text) returns text language sql immutable set search_path = public as $$
  select
    lower(
      translate(
...
create function tokenize(t text) returns text[] language sql immutable set search_path = public as $$
...
create function resolve_model(q text) returns text language plpgsql stable set search_path = public as $$
declare
  tok text[] := tokenize(q);
  candidate text;
begin
...
create function resolve_part(q text) returns text language plpgsql stable set search_path = public as $$
declare
  tok text[] := tokenize(q);
  p text;
begin
...
drop function if exists search_parts(text, int);
...
create function split_models(s text) returns text[] language sql immutable set search_path = public as $$
  select regexp_split_to_array(replace(s, ' ', ''), '/');
$$;

-- Helper to fetch all parts for a model_code via compat table
drop function if exists parts_for_model(text, int);
