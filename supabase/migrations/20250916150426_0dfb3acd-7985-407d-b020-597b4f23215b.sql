-- =============================================================================
-- STIHL Assistente Virtual - Configuração do Banco (CORRIGIDA)
-- =============================================================================
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        COALESCE(
            CASE WHEN p.part_code ILIKE ('%' || part_q || '%') THEN 0.9::REAL
                 ELSE similarity(p.description, part_q)::REAL
            END, 0.0::REAL
        ) as rank
...
INSERT INTO public.attendant_prompts (role, content, version, is_active) VALUES (
    'system',
    'Você é um especialista em peças STIHL. Responda de forma humanizada e direta.

REGRAS:
- NUNCA mencione: pdf, manual, arquivo, link, download
- Priorize: preço em BRL, código, descrição, compatibilidade  
- Se não souber, ofereça atendimento humano
- Use apenas dados do catálogo

FORMATO:
"O valor do [peça] da [modelo] é R$ [valor]
Código: [código]
Descrição: [descrição]"',
    '1.0',
    true
